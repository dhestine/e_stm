#include "i2c_driver.h"


/*
 * Pinout for the Nucleo-F411RE
 * PB8 ------ SCL
 * PB9 ------ SDA
 * I am using those for I2C1
 */

/*
 * From 18.6.8 in the reference manual
 * CCR = fclk/(2fscl)
 * CCR = 16E6/(2*100E3)
 * CCR = 80 or 0x50
 */

/*
 * From 18.6.9 in the reference manual
 * F = 16 MHz -> T = 62.5 ns
 * F = 100 kHz -> T
 * TRise = 1000 ns/ 62.5 ns +1
 * TRise = 17
 */

#define I2C_100kHz			0x50 // Clock control register value
#define Sm_MAX_RISE_TIME	17 // Maximum allowed rise time

void I2C1_init(void) {

    // Enable the clock for GPIO
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;

    // Configure PB8 and PB9 as Alternate Function (AF4)
	GPIOB->MODER &= (~GPIO_MODER_MODER8_Msk) | ~(GPIO_MODER_MODER9_Msk);
	GPIOB->MODER |= GPIO_MODER_MODER8_1 | GPIO_MODER_MODER9_1;


	GPIOB->AFR[1] |= (4U<<GPIO_AFRH_AFSEL8_Pos);
	GPIOB->AFR[1] |= (4U<<GPIO_AFRH_AFSEL9_Pos);


    // Set pins to open-drain and enable pull-ups
	GPIOB->OTYPER |= (GPIO_OTYPER_OT8 | GPIO_OTYPER_OT9);
	GPIOB->PUPDR |= (GPIO_PUPDR_PUPD8_0 | GPIO_PUPDR_PUPD9_0);

	// Enable clock for 12C
	RCC->APB1ENR |= RCC_APB1ENR_I2C1EN;

	// Reset 12C1 to clear any previous configuration
	I2C1->CR1 |= I2C_CR1_SWRST;
    I2C1->CR1 &= ~I2C_CR1_SWRST_Msk;

    // TO  configure clock frequency
	I2C1->CR2 |= (1U << 4);

	// Set the 12C clock speed to 100 kHz
	I2C1->CCR = I2C_100kHz;

	// Set the maximum rise time
	I2C1->TRISE = Sm_MAX_RISE_TIME;

	// Enable the 12C peripheral
	I2C1->CR1 |= I2C_CR1_PE;

}

void I2C1_readByte(char saddr, char maddr, char* data) {

	volatile int tmp; //Dummy variable for status register reads

	// Wait for the 12C bus to be idle
	while(I2C1->SR2 & I2C_SR2_BUSY){}

	// send a start condition
	I2C1->CR1 |= I2C_CR1_START;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_SB)){}

	// Send the slave address
	I2C1->DR = (saddr << 1);

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_ADDR)){}

	tmp = I2C1->SR2;

	// Send the data address
    I2C1->DR = maddr;

    // Wait for the data byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_TXE)){}

	// Send a start condition
	I2C1->CR1 |= I2C_CR1_START;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_SB)){}

	// Send the slave address with the read bit set
	I2C1->DR = saddr << 1 | 1;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_ADDR)){}

	// Clear the acknowledge bit
	I2C1->CR1 &= ~I2C_CR1_ACK_Msk;

	// Get the status register
	tmp = I2C1->SR2;

	// Send a stop condition
	I2C1->CR1 |= I2C_CR1_STOP;

	// Wait for the data byte to be received
	while(!(I2C1->SR1 & I2C_SR1_RXNE)){}

	// Receive the data byte
	*data++ = I2C1->DR;
	(void)tmp;

}

void I2C1_burstRead(char saddr, char maddr, int n, char* data) {
	volatile int tmp;

	// Wait for the 12C bus to be idle
	while(I2C1->SR2 & I2C_SR2_BUSY){}

	// Send a start condition
	I2C1->CR1 |= I2C_CR1_START;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_SB)){}

	// Send the slave address
	I2C1->DR = (saddr << 1);

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_ADDR)){}

	// Get the status register
	tmp = I2C1->SR2;

	// Wait for the data byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_TXE)){}

	// Send the data address
	I2C1->DR = maddr;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_TXE)){}

	// Send the slave address with the read bit set
	I2C1->CR1 |= I2C_CR1_START;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_SB)){}

	// Enable the acknowledge bit for reading
	I2C1->DR = saddr << 1 | 1;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_ADDR)){}

	// Get the status register
	tmp = I2C1->SR2;

	// Enable the acknowledge bit for reading
	I2C1->CR1 |= I2C_CR1_ACK;

	// Read n bytes from the slave device
	while(n > 0U){

		if(n == 1U){
			I2C1->CR1 &= ~I2C_CR1_ACK;

			I2C1->CR1 |= I2C_CR1_STOP;

			// Wait for the data byte to be received
			while(!(I2C1->SR1 & I2C_SR1_RXNE)){}

			// Receive the data byte
			*data++ = I2C1->DR;
			break;
		}

		// Otherwise, read the data byte and decrement
		else{

			// Wait for the data byte to be received
			while(!(I2C1->SR1 & I2C_SR1_RXNE)){}

			// Received the data byte
			*data++ = I2C1->DR;
			n--;
		}
	}

	// Discard the status register value
	(void)tmp;
}

void I2C1_burstWrite(char saddr, char maddr, int n, char* data) {

	volatile int tmp;

	// Wait for the 12C bus to be idle
	while(I2C1->SR2 & I2C_SR2_BUSY){}

	// Send a start condition
	I2C1->CR1 |= I2C_CR1_START;

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_SB)){}

	// Send the slave address
	I2C1->DR = (saddr << 1);

	// Wait for the address byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_ADDR)){}

	// Get the status register
	tmp = I2C1->SR2;

	while(!(I2C1->SR1 & I2C_SR1_TXE)){}

	// Send the data address
	I2C1->DR = maddr;

	// Send n data bytes to the slave device
	for(int i = 0; i < n; i++){

		// Wait for the data byte to be sent
		while(!(I2C1->SR1 & I2C_SR1_TXE)){}

		// Send the data byte
		I2C1->DR = *data++;

	}

	// Wait for the last byte to be sent
	while(!(I2C1->SR1 & I2C_SR1_BTF)){}

	// Send a stop condition
	I2C1->CR1 |= I2C_CR1_STOP;

	// Discard the status register value
	(void)tmp;


}
